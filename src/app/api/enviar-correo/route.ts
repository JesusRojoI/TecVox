...
export async function POST(request: Request) {
  try {
    ...
    const { to, orderData, language = 'es', currency = 'MXN', exchangeRate = 1 } = body;
    
    // Formateador dinámico
    const formatCurrency = (amount: number) => {
      const isUsd = currency === 'USD';
      // Convertimos el monto dividiéndolo entre el exchangeRate si es USD
    ...
    // Generamos las filas de la tabla usando formatCurrency
    const productosHTML = orderData.productos
      .map((p: any) => `
        <tr>
          <td style="padding:8px;border-bottom:1px solid rgba(59,130,246,0.2);color:#d1d5db;">
            ${p.nombre} ${p.sku ? `(SKU: ${p.sku})` : ''} × ${p.cantidad}
          </td>
          <td style="padding:8px;border-bottom:1px solid rgba(59,130,246,0.2);text-align:right;color:#60a5fa;">
      ...
      <div style="font-family:'Inter',Arial,sans-serif;max-width:600px;margin:0 auto;background-color:#121212;border-radius:12px;overflow:hidden;">
        ...
        <div style="padding:30px;color:#d1d5db;">
          ...
          <div style="margin-top:20px;padding:20px;background:rgba(30,64,175,0.1);border-radius:8px;border:1px solid rgba(59,130,246,0.2);">
            <p style="color:#d1d5db;"><strong>${isEnglish ? 'Subtotal:' : 'Subtotal:'}</strong> <span style="color:#60a5fa;">${formatCurrency(orderData.subtotal)}</span></p>
            ${orderData.descuento > 0 ? `<p style="color:#d1d5db;"><strong>${isEnglish ? 'Discount:' : 'Descuento:'}</strong> <span style="color:#ef4444;">-${formatCurrency(orderData.descuento)}</span></p>` : ''}
            <p style="color:#d1d5db;"><strong>${isEnglish ? 'Tax:' : 'Impuesto:'}</strong> <span style="color:#60a5fa;">${formatCurrency(orderData.impuesto)}</span></p>
            <p style="font-size:18px;color:#f8fafc;"><strong>${isEnglish ? 'Total:' : 'Total:'}</strong> <span style="color:#3b82f6;">${formatCurrency(orderData.total)}</span></p>
            ${orderData.cupon ? `<p style="color:#d1d5db;"><strong>${isEnglish ? 'Coupon used:' : 'Cupón utilizado:'}</strong> ${orderData.cupon}</p>` : ''}
          </div>
          
          <p style="color:#9ca3af;margin-top:20px;"><strong>${isEnglish ? 'Transaction:' : 'Transacción:'}</strong> ${orderData.transactionId}</p>
    ...
    await resend.emails.send({
      from: 'atencion@tecvox.com.mx',
      to: to,
      subject: isEnglish ? 'Purchase Confirmed! - TecVox' : '¡Compra confirmada! - TecVox',
      html: emailHTML,
    });

    // 2. Envío de copia al Administrador (Si está configurado)
    ...
    if (adminEmail) {
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:'Inter',Arial,sans-serif;max-width:600px;margin:0 auto;background:#121212;color:#d1d5db;border-radius:12px;overflow:hidden;">
            <div style="background:#1e40af;padding:20px;">
              <h2 style="color:#f8fafc;margin:0;">${isEnglish ? '📦 New Purchase' : '📦 Nueva compra'}</h2>
            </div>
            <div style="padding:20px;">
              <p><strong>${isEnglish ? 'Customer:' : 'Cliente:'}</strong> ${orderData.nombre}</p>
              <p><strong>Total:</strong> <span style="color:#3b82f6;">${formatCurrency(orderData.total)}</span></p>
            </div>
            ${emailHTML}
          </div>
        `,
